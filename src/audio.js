...
const Audio2 = (() => {
  ...
  const SFX = {
    ...
    bossMuncul:  () => {
      ...
      tone(120, t, 1.8, 'sawtooth', 0.35, null, 35);
      noise(t, 1.6, 200, 0.4, 0.30, 'lowpass');
      tone(48, t + 0.1, 2.0, 'sine', 0.42);
      for (let i = 0; i < 6; i++) {
        noise(t + 0.15 + i * 0.12, 0.08, 1400 + i * 250, 6, 0.14, 'bandpass');
      }
    },
    bossSerang:  () => {
      const t = ctx.currentTime;
      tone(180, t, 0.45, 'sawtooth', 0.45, null, 30);
      tone(48, t + 0.04, 0.55, 'sine', 0.55);
      noise(t, 0.35, 180, 0.5, 0.45, 'lowpass');
      noise(t, 0.22, 3200, 3, 0.28, 'bandpass');
      for (let i = 0; i < 4; i++) {
        tone(320 - i * 60, t + i * 0.035, 0.10, 'square', 0.20, null, 50);
      }
    },
    bossKena:    () => {
      const t = ctx.currentTime;
      noise(t, 0.28, 2800, 2, 0.32, 'bandpass');
      tone(440, t, 0.20, 'square', 0.24, null, 110);
      tone(880, t + 0.02, 0.14, 'triangle', 0.20, null, 220);
      noise(t + 0.05, 0.2, 800, 1.0, 0.22, 'lowpass');
    },
    bossKalah:   () => {
      const t = ctx.currentTime;
      noise(t, 2.0, 1000, 0.6, 0.35, 'lowpass');
      [220, 165, 110, 55].forEach((f, i) => tone(f, t + i * 0.2, 1.4, 'sine', 0.28, null, f * 0.6));
    },
  ...
  const MUSIK = {
    ...
    boss:    { akar: 110, skala: 'minor', tempo: 0.28, pad: .16, arp: .090, bass: .26, drum: .28, glitch: .09, arpPola: [0, 1, 3, 1, 6, 1, 3, 0] },
  ...
  function tick(cfg) {
    ...
    if (bar % 8 === 0) {                                    /* pad akor */
      ...
      [0, 2, 4].forEach((iv, i) => {
        const o = ctx.createOscillator();
        o.type = (cfg === MUSIK.boss && i === 0) ? 'sawtooth' : (i === 0 ? 'triangle' : 'sine');
        o.frequency.value = nada(cfg, akar + iv, -1) * (1 + (i - 1) * 0.002);
        const f = ctx.createBiquadFilter(); f.type = 'lowpass'; f.frequency.value = cfg === MUSIK.boss ? 650 : 900; f.Q.value = .7;
        o.connect(f);
        env(f, t, cfg === MUSIK.boss ? 0.4 : 1.1, cfg.tempo * 7.5, cfg.pad, musicGain);
        o.start(t); o.stop(t + cfg.tempo * 9);
      });
    }
    if (bar % 4 === 0 && cfg.bass) {                        /* bass */
      const b = ctx.createOscillator(); b.type = cfg === MUSIK.boss ? 'sawtooth' : 'sine';
      b.frequency.value = nada(cfg, (step / 4) % 2 === 0 ? 0 : 3, -2);
      const fb = ctx.createBiquadFilter(); fb.type = 'lowpass'; fb.frequency.value = 280; fb.Q.value = 1.2;
      b.connect(fb);
      env(fb, t, 0.02, cfg.tempo * 2.8, cfg.bass, musicGain);
      ...
      if (cfg === MUSIK.boss) {
        ...
        env(sub, t, 0.01, cfg.tempo * 3.8, cfg.bass * 1.3, musicGain);
        sub.start(t); sub.stop(t + cfg.tempo * 4);
      }
    }
    if (cfg.arp) {                                          /* arpeggio */
      const d = cfg.arpPola[bar % cfg.arpPola.length];
      const o = ctx.createOscillator(); o.type = cfg === MUSIK.boss ? 'sawtooth' : 'triangle';
      o.frequency.value = nada(cfg, d, bar % 8 < 4 ? 0 : 1);
      const fa = ctx.createBiquadFilter(); fa.type = 'bandpass'; fa.frequency.value = 1200; fa.Q.value = 2;
      o.connect(fa);
      env(fa, t, 0.01, cfg.tempo * 0.9, cfg.arp, musicGain);
      o.start(t); o.stop(t + cfg.tempo);
    }
    if (cfg.drum) {                                         /* ketukan */
      if (bar % 4 === 0) { const k = ctx.createOscillator(); k.type = 'sine';
        k.frequency.setValueAtTime(140, t); k.frequency.exponentialRampToValueAtTime(35, t + .16);
        env(k, t, 0.003, 0.18, cfg.drum * 1.3, musicGain); k.start(t); k.stop(t + .25); }
      if (bar % 8 === 4) noise(t, 0.12, 1800, 1.4, cfg.drum * .8, 'bandpass', musicGain);
      if (cfg === MUSIK.boss && (bar % 2 === 1)) {
        noise(t, 0.03, 5000, 4, cfg.drum * 0.25, 'highpass', musicGain);
      }
    }
    if (cfg.glitch && Math.random() < 0.35) {               /* gangguan digital */
      for (let i = 0; i < 3; i++)
